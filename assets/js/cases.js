...
const VERTICAL_VIDEO_FILES = new Set(["miniapp_ai_copilot_demo.mp4", "Shorts HH2.mp4", "Дизайн.mp4"]);
...
const PROJECTS = {
  ...
  ragKnowledge: {
    ...
    title: "Корпоративная ИИ-база знаний",
    lead: "Система ищет релевантные материалы в базе документов, показывает источники и держит ответы в рамках загруженного контекста.",
    route: ["Документы", "Embeddings", "Pinecone", "Контекст", "Ответ"],
    role: "Загрузка базы знаний, векторный поиск, генерация ответа с привязкой к источнику и интерфейс в Telegram.",
    highlights: [
      "Pinecone хранит базу в формате чанков для точного поиска.",
      "Embeddings и LLM формируют ответ по найденному контексту.",
      "Когда релевантной информации нет, система предлагает ручную проверку или уточнение.",
    ],
    architecture: [
      "Ingestion: документы → chunking → embeddings → Pinecone.",
      "Chat: Telegram → embedding вопроса → vector search → контекст → ответ.",
      "n8n оркестрирует загрузку базы и рабочий поиск.",
    ],
    stack: ["Pinecone", "OpenAI Embeddings", "OpenRouter", "n8n", "Telegram API"],
    video: {
      file: "0317 (2).mp4",
      title: "Демо базы знаний",
      note: "Вопрос в Telegram, поиск по Pinecone и ответ с привязкой к источнику.",
    },
  },

  leadRadarSystem: {
    badge: "ИИ-скоринг + CRM",
    title: "Система авто-поиска лидов",
    lead: "Система мониторит hh.ru, FL.ru, Habr Career и Telegram, оценивает лиды через ИИ и отправляет лучшие варианты в работу.",
    route: ["Источник", "Скоринг", "Таблица", "Telegram", "Догрев"],
    role: "Сбор источников, ИИ-скоринг, дедупликация, доставка в CRM и Telegram-уведомления.",
    highlights: [
      "Мониторы по расписанию ищут релевантные вакансии и заказы.",
      "ИИ оценивает лиды и готовит черновик отклика.",
      "Подходящие лиды пишутся в таблицу и уходят в Telegram с оценкой и reasoning.",
    ],
    architecture: [
      "Сценарии источников запускаются по cron для hh.ru, Habr Career, FL.ru и Telegram.",
      "Внутренние данные дедуплицируют лиды и не дают обработать одну позицию дважды.",
      "После скоринга система пишет лид в CRM, шлёт уведомление и ведёт догрев.",
    ],
    stack: ["n8n", "hh.ru API", "OpenRouter", "Gemini Flash", "Google Sheets", "Telegram"],
    video: {
      file: "Shorts HH2.mp4",
      title: "Демо авто-поиска лидов",
      note: "Новые вакансии, ИИ-скоринг, сохранение в CRM и Telegram-уведомление.",
    },
  },
